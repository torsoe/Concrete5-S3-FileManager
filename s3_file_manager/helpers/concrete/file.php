...

// cant extend the f***ing function
		...
		public function getThumbnailRelativePath($prefix, $filename, $level) {
			...
			if (defined('S3_ENABLED') && S3_ENABLED == true){
				return $this->S3_getThumbnailSystemPath($prefix, $filename, $level);
			}else{
				return $this->C5_getThumbnailRelativePath($prefix, $filename, $level);
			}
		}

		public function C5_getThumbnailRelativePath($prefix, $filename, $level) {
			switch($level) {
				case 2:
		...
		public function getThumbnailSystemPath($prefix, $filename, $level, $createDirectories = false) {
			//start S3 tour if s3 is enabled
			if (defined('S3_ENABLED') && S3_ENABLED == true){
				return $this->S3_getThumbnailSystemPath($prefix, $filename, $level, $createDirectories);
			}else{
				return $this->C5_getThumbnailSystemPath($prefix, $filename, $level, $createDirectories);
			}
		}

		public function S3_getThumbnailSystemPath($prefix, $filename, $level, $createDirectories = false) {
			switch($level) {
				case 2:
					$base = S3_DIR_FILES_UPLOADED_THUMBNAILS_LEVEL2;
					break;
				case 3:
					$base = S3_DIR_FILES_UPLOADED_THUMBNAILS_LEVEL3;
					break;
				default: // level 1
					$base = S3_DIR_FILES_UPLOADED_THUMBNAILS;
					break;
			}
			...
			$hi = Loader::helper('file');
			$path = $this->mapSystemPath($prefix, $filename, false,false, $base);
			if (!file_exists($path)) {
				$filename = $hi->replaceExtension($filename, 'jpg');
				$path = $this->mapSystemPath($prefix, $filename, false,false, $base);
			}
		...
		public function C5_getThumbnailSystemPath($prefix, $filename, $level, $createDirectories = false) {
			switch($level) {
				case 2:
					$base = DIR_FILES_UPLOADED_THUMBNAILS_LEVEL2;
					break;
		...
		public function mapSystemPath($prefix, $filename, $createDirectories = false, $base = DIR_FILES_UPLOADED, $thumbailPath = false) {
			//start S3 tour if s3 is enabled
			if (defined('S3_ENABLED') && S3_ENABLED == true){
				return $this->S3_mapSystemPath($prefix, $filename, false,false,$thumbailPath);
			}else{
				return $this->C5_mapSystemPath($prefix, $filename, $createDirectories, $base);
			}
		}


		public function S3_mapSystemPath($prefix, $filename, $createDirectories = false, $base = DIR_FILES_UPLOADED , $thumbailPath = false){
			...
			if($thumbailPath)
				$path = S3_PATH.S3_DIR.$thumbailPath.'/' . $d1 . '/' . $d2 . '/' . $d3 . '/' . $filename;
			else
				$path = S3_PATH.S3_DIR.'/' . $d1 . '/' . $d2 . '/' . $d3 . '/' . $filename;

			return ltrim($path,'/');
		}

		public function C5_mapSystemPath($prefix, $filename, $createDirectories = false, $base = DIR_FILES_UPLOADED){
			if ($prefix == null) {
				$path = $base . '/' . $filename;
			} else {
				$d1 = substr($prefix, 0, 4);
